...
export default function CommunityPage() {
  ...
  const scrollCommunityToEnd = useStickToBottomSupportChat(scrollRef, [messages, pendingFiles])
  ...
  const send = () => {
    ...
    if (isApi && token) {
      void (async () => {
        try {
          const encoded =
            pendingFiles.length > 0 ? await encodeSupportAttachmentFiles(pendingFiles.map((p) => p.file)) : undefined
          ...
          const detail = e instanceof Error ? e.message : 'Попробуйте позже.'
          setMessages((m) => [
            ...m,
            {
              id: crypto.randomUUID(),
              role: 'assistant',
              text: `Не удалось отправить: ${detail}`,
              attachments: [],
              createdAt: Date.now(),
            },
          ])
  ...
  return (
    <div
      className="container flex flex-col pt-2 pb-2"
      style={{ height: 'calc(100dvh - 10.5rem)', minHeight: 320 }}
    >
      <div
        ref={scrollRef}
        className="flex-1 min-h-0 overflow-y-auto space-y-3 pr-0.5"
      >
        {messages.map((msg) => (
          <div
            key={msg.id}
          ...
          >
            <div
              className={`max-w-[88%] rounded-2xl px-3 py-2.5 shadow-sm border ${msg.role === 'user'
      ...
      {adminTypingPeer && isApi ? (
        <p className="shrink-0 px-0.5 pt-1 text-[11px] text-slate-500 dark:text-slate-400 italic">
      ...
      <div className="shrink-0 flex gap-2 items-end pt-2 border-t border-slate-200 dark:border-slate-700">
        ...
        <button
          ...
          className="shrink-0 w-11 h-11 rounded-xl bg-surface border border-slate-200 dark:border-slate-600 flex items-center justify-center text-slate-600 dark:text-slate-300 active:opacity-80"
        ...
        <textarea
          ...
          rows={2}
          className="flex-1 min-h-[44px] max-h-28 resize-none rounded-xl border border-slate-200 dark:border-slate-600 bg-surface px-3 py-2.5 text-sm text-slate-900 dark:text-slate-100 placeholder:text-slate-400 outline-none focus:ring-2 focus:ring-slate-400/40 dark:focus:ring-slate-500/40"
        />
        <button
          type="button"
          onClick={send}
